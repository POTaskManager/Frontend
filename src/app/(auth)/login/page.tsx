...
import { REDIRECT_PATH, useAuthStore } from '@/store/auth-store';
...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    // Check if user is already authenticated (cookies are set by backend)
    fetchUser();
  ...
  useEffect(() => {
    ...
    if (user) {
      const callbackUrl = (searchParams.get('callbackUrl') || REDIRECT_PATH) as Route;
  ...
  const onSubmit = async (values: LoginValues) => {
    ...
    try {
      ...
      // Backend sets cookies, fetch user to update store
      await fetchUser();
